...
public class GuiColorChooser extends GuiElement {
    public static final ResourceLocation ColorPickBaground = new ResourceLocation("potioncolorizer",
    ...
    public final GuiElement LumSlider;
    ...
    public GuiColorChooser(int x, int y, int WithAndHieght, int initialColor, Consumer<Integer> onChange) {
        ...
        Picker = new GuiRect(0, 1, WithAndHieght, WithAndHieght, 0xffffff).setOpacity(0.3f);
        LumSlider = new GuiRect(110, 0, 4, WithAndHieght, 0xffffff).setOpacity(0.3f);

        addChild(new GuiRect(120, 80, 20, 20,initialColor).setOpacity(1f));
        ...
        hoverIndicatorLum = new GuiRect(110, 1, 4, 4, 0xffff33).setOpacity(0.3f);
        ...
        currentIndicatorLum = new GuiRect(110, 1, 4, 4, 0xffffff);
    ...
    public int getColor() {
        int rgb = Color.HSBtoRGB(this.hsb[H], this.hsb[S], this.hsb[B]);
        return rgb;
    }
    ...
    public void draw(PoseStack matrixStack, int refX, int refY, int screenWidth, int screenHeight, int mouseX, int mouseY, float opacity) {
        addChild(new GuiRect(120, 80, 20, 20,rgb).setOpacity(1f));
        if (isDraggingPicker) {
            ...
            if (newSegmentY != valueY) {
                if (newSegmentX != valueX) {
                    ...
                    this.hsb[H] = Math.min(Math.max((mouseX - refX - x - 1) / (1f * indicatorMax), 0),1);
                    this.hsb[S] = Math.min(Math.max((mouseY - refY - y - 1) / (1f * indicatorMax), 0),1);
                    this.updateColor();
                    }
