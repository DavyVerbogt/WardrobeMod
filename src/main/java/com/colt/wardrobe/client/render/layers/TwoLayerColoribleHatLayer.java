...
import com.colt.wardrobe.client.render.models.TopTopTophatModel;
...
import static com.colt.wardrobe.client.render.layers.LayerDefenition.*;

public class TwoLayerColoribleHatLayer<T extends LivingEntity> extends RenderLayer<T, PlayerModel<T>> {

    public static AgeableListModel[] HatModels =  {topHatModel, topTopTophatModel};
    ...
    public TwoLayerColoribleHatLayer(RenderLayerParent<T, PlayerModel<T>> renderer, EntityModelSet modelSet) {
        super(renderer);

        for (AgeableListModel model : HatModels) {
    ...
    protected ResourceLocation getTextureLocation(T entity, String layer) {
        return switch (LayersConfig.LAYER_CHOICE.get()) {
            case TOP_HAT -> TopHatTexture = new ResourceLocation(Wardrobe.MOD_ID,
                    "textures/player/hats/tophat/top_hat" + layer + ".png");

                    case TOP_TOP_TOP_HAT-> TopTopTopHatTexture = new ResourceLocation(Wardrobe.MOD_ID,
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLight, T entity, float limbSwing,
        ...
        if (TurnTophatOn) {
            colorRenederer(poseStack, buffer, packedLight, entity, limbSwing, limbSwingAmount, partialTick, ageInTicks,
                    netHeadYaw, headPitch);
        }
    }

    public void colorRenederer(PoseStack poseStack, MultiBufferSource buffer, int packedLight, T entity,
        ...
        // Render Layer 2
        this.renderModelPerLayer(poseStack, buffer, packedLight, false, topHatModel, entity, r1, g1, b1, alpha1,
                "_layer_1");
    }

    private void renderModelPerLayer(PoseStack poseStack, MultiBufferSource buffer, int packedLight, boolean glint,
            net.minecraft.client.model.Model model, Entity entity, float r, float g, float b, Float aplha,
            @Nullable String textureLayer) {
        renderModel(poseStack, buffer, packedLight, glint, model, r, g, b, aplha,
                this.getTextureLocation((T) entity, textureLayer));
    }
